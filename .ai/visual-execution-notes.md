...
## 2025-12-10 · Phase 8 Validation & Observability

- **Backend verification:** Added `workflows.service.spec.ts` coverage for the unified run payload endpoint. Tests assert that schedule triggers persist to `workflow_runs` and that idempotency keys generate stable `shipsec-run-*` identifiers. This guards the dispatcher workflow against duplicate scheduled runs.
- **Frontend verification:** `runStore.test.ts` now ensures trigger metadata defaults (Manual/Scheduled/API labels) and input previews are always present so badges render even when the backend omits labels.
- **Schedule module tests:** New `backend/src/schedules/__tests__/schedules.service.spec.ts` exercises schedule creation, validation, Temporal pause/resume, and manual trigger paths against the in-memory repository + dispatcher args to ensure we never regress dispatch metadata or node overrides.
- **Schedule UI tests:** `frontend/src/store/__tests__/scheduleStore.test.ts` covers cache freshness, pause/resume mutations, inline run-now, and filter state so the Schedules page + workflow CTA can trust the shared slice under live data.
- **Manual QA checklist:**
  1. `bun --cwd backend test backend/src/workflows/__tests__/workflows.service.spec.ts`
  2. `bun --cwd frontend test frontend/src/store/__tests__/runStore.test.ts`
  3. `bun --cwd backend test backend/src/schedules/__tests__/schedules.service.spec.ts`
  4. `bun --cwd frontend test frontend/src/store/__tests__/scheduleStore.test.ts`
  5. Start the platform (`docker compose -p shipsec up -d`, `pm2 start pm2.config.cjs`), manually run a workflow, and confirm the info bar + dropdown show `👤 Manual run` with a green `Completed` badge anchored bottom-right.
  6. From the Schedules page create a cadence with unique runtime inputs, Pause → Resume it, and hit “Run now”; timeline rows and inspector badges must show `🕐 Scheduled run` plus the schedule name.
  7. Exercise the workflow-level schedule drawer: confirm the inline chips reflect pause/resume immediately and that the new drawer run creates another `🕐` run without duplicating cache entries.
  8. Exercise the Run Selector trigger filter to ensure switching between Manual/Scheduled reuses the cached `GET /workflows/runs` payload (no duplicate network calls).
- **Schedule health monitoring:** `GET /api/v1/schedules` exposes `status`, `lastRunAt`, `nextRunAt`, and Temporal status snapshots. Operators should periodically run `curl -s http://localhost:3211/api/v1/schedules | jq '.[] | {name,status,nextRunAt}'` and investigate any rows where `status === "error"` via `GET /api/v1/schedules/:id`. This data is the basis for future alerts (Loki/Prometheus) on stuck cadences.
- **Documentation:** When validating new features, append manual verification notes (hot reload, rerun badge placement, trigger filters) to PR descriptions so QA can replay the scenarios without combing through commits.
