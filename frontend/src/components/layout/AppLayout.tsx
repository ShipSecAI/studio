...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  // Auto-collapse sidebar when opening workflow builder or webhook editor, expand for other routes
  ...
  useEffect(() => {
    if (isMobile) {
      ...
      // Only collapse for workflow builder (not the list page) and webhook editor
      // /workflows = list page (keep sidebar open)
      // /workflows/new or /workflows/:id = builder (collapse sidebar)
      // /webhooks/:id = webhook editor (collapse sidebar)
      const isWorkflowBuilder =
        location.pathname.startsWith('/workflows/') && location.pathname !== '/workflows/';
      const isWebhookEditor = location.pathname.startsWith('/webhooks/');
      const shouldCollapse = isWorkflowBuilder || isWebhookEditor;
    ...
    <SidebarContext.Provider value={sidebarContextValue}>
      ...
      <div className="flex h-screen bg-background overflow-hidden">
        ...
        >
          ...
          <SidebarContent className="py-0 flex flex-col h-full overflow-hidden">
            {/* Fixed Top Section: Navigation + Command Palette */}
            <div className="flex-shrink-0">
            ...
            {sidebarOpen && (
              <div className="px-2 mt-4 pt-4 border-t border-border/40 flex-1 overflow-y-auto min-h-0">
