...
# ==========================================================================
# GCS FUSE Volume Support (for K8s job pods)
# ==========================================================================

# GCS bucket for job volumes (auto-cleanup after 7 days)
...
# Job runner SA → volumes bucket access
resource "google_storage_bucket_iam_member" "job_runner_volumes" {
...
# Existing storage SA also needs access to volumes bucket (worker reads/writes via SDK)
resource "google_storage_bucket_iam_member" "storage_volumes" {
  bucket = google_storage_bucket.volumes.name
  role   = "roles/storage.objectUser"
  member = "serviceAccount:${google_service_account.storage.email}"
}
